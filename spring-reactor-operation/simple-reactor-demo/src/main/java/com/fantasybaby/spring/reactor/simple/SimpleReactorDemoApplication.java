...
//		Flux.range(1, 10).doOnComplete(()-> System.out.println("----------")).subscribe(System.out::println);
		...
		}, throwable -> System.out.println("error"),()->latch.countDown());
	...
	private void testPublisherAndSchedule(){
		Scheduler scheduler = Schedulers.newElastic("publish on elastic", 10);
		Flux.range(1, 6)
				.publishOn(scheduler)
...
//				.onErrorReturn(-1)
				.subscribe(i -> log.info("Subscribe {}: {}", Thread.currentThread(), i),
						...
						() -> {log.info("Subscriber COMPLETE");scheduler.dispose();}
//						,s -> s.request(4)
				);
	}
	@Override
